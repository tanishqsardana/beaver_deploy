...
1. **Upload Dam Locations.** Coordinates of the dam locations (or BDA, etc.) should be provided in a CSV (containing columns for lattitude and longitude) or as a GeoJSON (GeoJSONs can be generated through a map interface [here](https://geojson.io/) if desired). Users then select the year for which they would like to perform the analysis by using a drop down menu. Note that due to data restrictions we can only offer analysis of data from April 2017 through the end of 2024 at this time, and some years and locations may not have evapotranspiration data. 
